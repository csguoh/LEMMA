...
class DecoderLayer(nn.Module):
    ...
    def __setstate__(self, state):
        if 'activation' not in state:#fasle
    ...
    def forward(self, query, content, memory, query_mask: Optional[Tensor] = None, content_mask: Optional[Tensor] = None,
        ...
        query_norm = self.norm_q(query)#self-attn: q
        content_norm = self.norm_c(content)#self_attn: k v
...
class Encoder(VisionTransformer):
    ...
    def forward(self, x):
        # Return all tokens
        return self.forward_features(x) # 直接使用timm的vision transformer
