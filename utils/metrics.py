...
def get_string_abinet(v_res,abi_charset):
    # 输入是文本识别结果的logits，输出是字符串列表
    # TODO 这里只使用v_res，只后还可以再加上a_res
    ...
    for o in out:
        text = abi_charset.get_text(o.argmax(dim=1), padding=False, trim=False)
        text = text.split(abi_charset.null_char)[0]  # end at end-token 本质上也是实现了trim的功能
        pt_text.append(text)
        pt_scores.append(o.max(dim=1)[0])
        pt_lengths.append(min(len(text) + 1, abi_charset.max_length))  # one for end-token
...
def get_string_parseq(output_dict,tokenizer):
