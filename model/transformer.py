...
class TransformerDecoder_DiffKV(Module):
    r"""TransformerDecoder is a stack of N decoder layers
    ...
    def __init__(self, decoder_layer, num_layers, norm=None):
        super(TransformerDecoder_DiffKV, self).__init__()
        self.layers = _get_clones(decoder_layer, num_layers)
        self.num_layers = num_layers
        self.norm = norm
    ...
    def forward(self, tgt, memory, memory2=None, tgt_mask=None,
        ...
        for mod in self.layers:
            output = mod(output, memory[cnt], memory2=memory2, tgt_mask=tgt_mask,
                         memory_mask=memory_mask, memory_mask2=memory_mask2,
                         tgt_key_padding_mask=tgt_key_padding_mask[cnt],
                         memory_key_padding_mask=memory_key_padding_mask[cnt],
                         memory_key_padding_mask2=memory_key_padding_mask2)
            cnt = cnt + 1
        if self.norm is not None:
            output = self.norm(output)
...
class TransformerDecoderLayer_QKV(Module):
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1,
                 activation="relu", self_attn=True, siamese=False, debug=False):
    ...
    def forward(self, query, key, value=None,tgt_mask=None, memory_mask=None,
                tgt_key_padding_mask=None, memory_key_padding_mask=None,
                memory2=None, memory_mask2=None, memory_key_padding_mask2=None):

        tgt2, attn2 = self.multihead_attn(query, key, value, attn_mask=memory_mask,
                                          key_padding_mask=memory_key_padding_mask)
        if self.debug: self.attn2 = attn2
...
class TransformerDecoder_QKV(Module):
    __constants__ = ['norm']
    def __init__(self, decoder_layer, num_layers, norm=None):
        super(TransformerDecoder_QKV, self).__init__()
        self.layers = _get_clones(decoder_layer, num_layers)
        self.num_layers = num_layers
        self.norm = norm

    def forward(self, query, key, value=None, tgt_mask=None,
                memory_mask=None, memory_mask2=None, tgt_key_padding_mask=None,
                memory_key_padding_mask=None, memory_key_padding_mask2=None):
        ...
        # 3层 Decoder Layer
        for mod in self.layers:
            output = mod(output,key,value, memory2=None, tgt_mask=tgt_mask,
                         memory_mask=memory_mask, memory_mask2=memory_mask2,
                         tgt_key_padding_mask=tgt_key_padding_mask,
                         memory_key_padding_mask=memory_key_padding_mask,
                         memory_key_padding_mask2=memory_key_padding_mask2)
